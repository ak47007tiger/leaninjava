...
public class DoprintProxy implements InvocationHandler {
    ...
    public IDoAdd getDoAdd(){
    	IDoAdd doAdd = null;
    	doAdd = (IDoAdd) Proxy.newProxyInstance(IDoAdd.class.getClassLoader(), new Class[]{IDoAdd.class}, new DoAddHandler());
    	return doAdd;
    }
    ...
    public IDoprint create() {
    	...
    	//代理签约
        returnResource = (IDoprint) Proxy.newProxyInstance(IDoprint.class.getClassLoader(), new Class[]{ IDoprint.class }, this);
        return returnResource;
    }
    /**
     * 所有的方法都要经过代理，那么方法执行的结果就可以和调用时的预测结果不同了
     * 很强的灵活性
     * 一个对象只有一个代理
     * 一个代理可以代理多个对象
     */
