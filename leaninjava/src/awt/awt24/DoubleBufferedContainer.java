...
public class DoubleBufferedContainer extends Container {
	...
	public void paintComponent(Component comp, boolean update) {
		...
		if (wpg != null) {
			try {
				compGraphics = wpg.create(bounds.x, bounds.y, bounds.width,
						bounds.height);
	...
	public void moveComponent(Component comp, Point newLoc, boolean update) {
		...
		if (update)
			blitWorkplaceToScreen(oldBounds.union(comp.getBounds()));
	}
